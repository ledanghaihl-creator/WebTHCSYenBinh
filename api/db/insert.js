...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!table || !item) {
      return res.status(400).json({ success: false, message: 'Missing table or item' });
    }

    const { data, error } = await supabaseService.from(table).insert([item]).select();
    if (error) {
      console.error(`Lỗi Vercel Serverless insert vào ${table}:`, error);
